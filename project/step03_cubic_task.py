...
def sysCall_thread():
    ...
    log_once(sim, aux, f"[DEMO] Task-space cubic (pos-only, fixed R) — cases={NUM_CASES}")
    ...
    for case in range(1, NUM_CASES+1):
        ...
        # เวลาเดิน (เคารพความเร่งเชิงเส้นสูงสุด)
        T_AB = choose_T_task_with_accel(pA, pB, ALIN_MAX, T_HINT)
        ...
        # วิ่ง A -> B ใน task-space cubic (orientation คงเดิม = R_hold)
        q_end = run_leg_task_cubic(sim, aux, hdl_j, hdl_end,
                                   qA, pA, pB, R_hold, T_AB,
                                   f"[Case {case}] A→B (task-cubic)")
